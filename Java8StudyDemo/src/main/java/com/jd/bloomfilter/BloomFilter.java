...
public class BloomFilter<E> implements Serializable {
    ...
    public BloomFilter(double falsePositiveProbability, int expectedNumberOfElements) {
        ...
        System.out.println("falsePositiveProbability = " + falsePositiveProbability);
        System.out.println("expectedNumberOfElements = " + expectedNumberOfElements );
        System.out.println("c:"+Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2)))/ Math.log(2));
        System.out.println("n:"+expectedNumberOfElements);
        System.out.println("k:"+(int) Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))));
    }
    ...
    public void add(E element) {
       // add(element.toString().getBytes(charset));
        Random r = new Random(element.hashCode());
        for (int x = 0; x < k; x++) {
            bitset.set(r.nextInt(bitSetSize), true);
        }
...
//        return contains(element.toString().getBytes(charset));
        Random r = new Random(element.hashCode());
        for (int x = 0; x < k; x++) {
            if (!bitset.get(r.nextInt(bitSetSize)))
                return false;
        }
        return true;
    }
